/**
 * generic – the "import" subcommand implementation
...
Result agent_import(
    ...
    fs::path from_state = fs::path(from_dir) / "state";
    fs::path to_state   = fs::path(workspace_dir) / "state";

    if (!fs::exists(from_state)) {
        ERROR_FMT_("Source state directory not found: {}", from_state.string());
        return Result(1);
    }
    ...
    for (const auto& entry : fs::directory_iterator(from_state)) {
        ...
        NOTICE_FMT_("Imported: {}", entry.path().filename().string());
        ++copied;
    }
